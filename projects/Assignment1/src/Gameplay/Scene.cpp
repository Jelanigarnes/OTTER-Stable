...
namespace Gameplay {
	Scene::Scene() :
		...
		ambientSpecToggle(false),
		ambientLightOnly(true),
		...
		if (IsPlaying) {
			...
			if (InputEngine::GetKeyState(GLFW_KEY_3) == ButtonState::Pressed) {
				if (SpecularToggle) {
					...
					FindObjectByName("Character1")->Get<RenderComponent>()->SetMaterial(Character1NoSpecular);
					SpecularToggle = false;
				}
				else
				{
					FindObjectByName("Character1")->Get<RenderComponent>()->SetMaterial(Character1Material);
					SpecularToggle = true;
				}
			}
			//Ambient + Specular
			if (InputEngine::GetKeyState(GLFW_KEY_4) == ButtonState::Pressed) {
				if (ambientSpecToggle) {
					SetAmbientLight(glm::vec3(0.0f, 0.0f, 0.0f));					
					FindObjectByName("Character1")->Get<RenderComponent>()->SetMaterial(Character1NoSpecular);
					ambientSpecToggle = false;
				}
